...
@test "deploy project with go source error" {
  run $NIM project deploy $BATS_TEST_DIRNAME --remote-build --include test-remote-build-errs/bad-src
  assert_failure
...
@test "deploy project with improper generated .include" {
  run $NIM project deploy $BATS_TEST_DIRNAME --remote-build --include test-remote-build-errs/bad-include
  assert_failure
  assert_line -p "Illegal use of '..' in an '.include' file"
}

@test "deploy project with misbehaving build script" {
  run $NIM project deploy $BATS_TEST_DIRNAME --remote-build --include test-remote-build-errs/bad-script
  assert_failure
  assert_line -p "Output of failed build"
